// /app/api/courseregister/route.ts
...
export async function scrapeCourseRegister(client: ReturnType<typeof wrapper>) {
    const response = await client.get(COURSE_REGISTER_URL, { responseType: 'arraybuffer' });
    ...
    const titleRow = $('#ctl00_ctl00_contentPane_MainPanel_MainContent_gvRegisteredList_DXTitle td.dxgvTitlePanel_Mulberry')
        .first()
        .text();
...
export async function POST(req: NextRequest) {
    try {
        ...
        const data = await scrapeCourseRegister(client);
